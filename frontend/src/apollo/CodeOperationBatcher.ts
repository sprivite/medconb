import {FetchResult, NextLink, Observable, ObservableSubscription, Operation} from '@apollo/client'
...
// into one query.
export class CodeOperationBatcher {
  ...
  public consumeQueue(key = ''): (Observable<FetchResult> | undefined)[] | undefined {
    ...
    const batchedObservable = this.batchHandler(operations, forwards) || Observable.of()
    ...
    batch.subscription = batchedObservable.subscribe({
      next: (results) => {
        ...
        //   results = [results]
        // }
        nexts.forEach((_nexts, index) => {
          const code = find(results.data!.codes, {id: operations[index].variables['codeID']})
          const __payload = {
            ...code,
            __typename: 'Code',
            children: (code.children ?? []).map((c: any) => ({...c, __typename: 'Code'})),
          }
          _nexts.forEach((next) =>
            next({
              data: {
        ...
        if (nexts.length !== results.data!.codes.length) {
          const error = new Error(
            `server returned results with length ${results.data!.codes.length}, expected length of ${nexts.length}`,
          )
